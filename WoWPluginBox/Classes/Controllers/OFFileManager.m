...
{
	return [[OFFileManager pathForApplicationSupportDirectory] stringByAppendingPathComponent:path];
}
...
{
	return [[OFFileManager pathForCachesDirectory] stringByAppendingPathComponent:path];
}
...
{
	return [[OFFileManager pathForDocumentsDirectory] stringByAppendingPathComponent:path];
}
...
{
	return [[OFFileManager pathForMainBundleDirectory] stringByAppendingPathComponent:path];
}
...
{
	return [[OFFileManager pathForTemporaryDirectory] stringByAppendingPathComponent:path];
}
...
+ (long long)sizeOfDirectoryAtPath:(NSString *)path
{
	...
	for (int i = 0; i < [array count]; i++) {
		...
		if (!([fileManager fileExistsAtPath:fullPath isDirectory:&isDir] && isDir)) {
			...
			size += [self sizeOfDirectoryAtPath:fullPath];
		}
	}
